...
class TestAutorizedUser(TestCase):
    ...
    def setUp(self):
        ...
        self.response_post_create = self.client.post(
            "/new/", {"text": self.testtext1}, follow=True)
    ...
    def test_have_new_post(self):
        ...
        self.assertContains(self.response_index,
                            self.testtext1, status_code=200)
        self.assertContains(self.response_profile,
                            self.testtext1, status_code=200)
        self.assertContains(self.response_post,
                            self.testtext1, status_code=200)
    ...
    def test_edit_post(self):
        ...
        self.client.post(
            reverse("post_edit", kwargs={
                    "username": self.testusername, "post_id": 1}),
            {"text": self.testtext2},
        )
...
class TestCache(TestCase):
    ...
    def test_cache_index(self):
        # Создаем пост
        response_post_create = self.client.post(
            "/new/", {"text": self.testtext1}, follow=True)
...
class TestImages(TestCase):
    ...
    def setUp(self):
        # Регистрируем и логиним пользователя, создаем свою картинку
        self.response_signup = self.client.post(
            "/auth/signup/", {"username": self.testusername, "email": self.testemail,
                              "password1": self.password, "password2": self.password}, follow=True
        )
        ...
        # Создают новый пост
        self.response_create_post = self.client.post(
            "/new/", {"text": self.testtext1}, follow=True)
        # Редактирую новый пост и прикладываю к нему картинку, созданную выше
    ...
    def test_safe_download(self):
        ...
        with open('readme.md', 'rb') as file:
            response = self.client.post(
                '/new/', {'text': self.testtext1, 'image': file})
            self.assertFormError(
                response, 'form', 'image', 'Upload a valid image. The file you uploaded was either not an image or a corrupted image.')
...
class TestFollow(TestCase):
    ...
    def test_new_post_in_follow_index(self):
        ...
        self.assertContains(response_follow_index,
                            self.testtext1, status_code=200)
        self.client.get('/username1/unfollow/', follow=True)
        response_unfollow_index = self.client.get('/follow/')
        self.assertNotContains(response_unfollow_index,
                               self.testtext1, status_code=200)

    def test_create_comment_for_post(self):
        self.client.post("/username1/1/comment/",
                         {"text": self.testtext2}, follow=True)
